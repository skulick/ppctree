...
# also allow for added _NT at end of bare label
...
RE_LABEL_TRACE = re.compile(r'^(?P<bare>[A-Z]+\$?(_NT)?)(?P<ftags>(-[A-Z]+)+)*(-(?P<trace>[0-9]+))*$')
RE_LABEL_GAP = re.compile(r'^(?P<bare>[A-Z]+\$?(_NT)?)(?P<ftags>(-[A-Z]+)+)*(=(?P<gap>[0-9]+))*$')
